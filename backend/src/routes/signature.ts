...
import {
  analyzeDocumentForStamp,
  type DocumentVerificationResult,
} from "../services/document-verification.service";
import { documentVerificationQueue } from "../services/email/ai/queues";
import { loadPdf, renderPageToBase64 } from "../services/email/ai/pdfExtract";

const router = Router();

// ─── Multer Config ────────────────────────────────────────────────────────────

const signatureUploadsDir =
  process.env.SIGNATURE_UPLOADS_DIR ?? "uploads/signatures";
...
router.post(
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    try {
      const pageStrategy = (req.body.pageStrategy as string) || "last";

      const result = await analyzeDocumentForStamp(
        file.path,
        file.mimetype,
        {
          pageStrategy: pageStrategy as any,
...
router.post(
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      const pageStrategy =
        (req.body.pageStrategy as string) || "last";
      ...
      return res.status(500).json(
        fail(error.message || "Erreur lors de la vérification du document."),
      );
    }
  },
);
...
router.post(
  "/verify-attachment/:attachmentId",
  requireRole(["admin", "agent_commercial", "technique"]),
  async (req: AuthRequest, res: Response) => {
    try {
...
router.post(
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      const { documentIds, documentType = "tender_document" } = req.body as {
        documentIds: string[];
        documentType?: string;
      };
      ...
      for (const docId of documentIds) {
        try {
          ...
          if (documentType === "tender_document") {
            ...
            const att =
              await db.query.supplierResponseAttachments.findFirst({
