...
export async function sendCommercialOfferService({
  ...
  supplierIds: string[];
  //signature?: string;
  attachments?: { filename: string; path: string }[]; // ✅ array
}) {
  ...
  if (cleanSupplierIds.length === 0) throw new Error("No suppliers selected");
  ...
  if (supplierList.length === 0) throw new Error("No valid suppliers found");
  ...
  for (const supplier of supplierList) {
    try {
      if (!supplier.email) throw new Error("Supplier email missing");
      ...
      await db
        ...
        .where(
          and(
            eq(offerSuppliers.offerId, offerId),
