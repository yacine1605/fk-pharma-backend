...
export async function createAndSendOffer(
  ...
  if (dto.lots?.length) {
    await db.insert(offerLots).values(
      dto.lots.map((lot) => ({
        offerId: offer.id,
  ...
  // 5. SAVE ATTACHMENTS
  if (dto.attachments?.length) {
    await db.insert(offerAttachments).values(
  ...
  if (selectedSuppliers.length) {
    await db.insert(offerSuppliers).values(
        ...
        await db
          ...
          .where(
            and(
              ...
              eq(offerRecipients.recipientId, user.id),
            ),
          );
        details.push({
          type: "user",
  ...
  const [offer] = await db
    ...
    .values({
      ...
      technicalDepartmentDepositDate:
        dto.technicalDepartmentDepositDate ?? null,
    })
...
async function withRetry<T>(fn: () => Promise<T>, retries = 3): Promise<T> {
  ...
  for (let i = 0; i < retries; i++) {
    try {
      ...
      lastError = e;
      if (
        (e?.cause?.code === "ECONNRESET" || e?.code === "ECONNRESET") &&
        i < retries - 1
      ) {
