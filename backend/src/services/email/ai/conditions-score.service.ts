 ...
 */
export function calculateConditionsScore(params: ConditionsScoreInput): number {
 ...
 */
export function calculateConditionsScoreDetailed(
  params: ConditionsScoreInput,
  ...
  if (validityDays !== null) {
    if (validityDays >= 60) validityScore = 40;
    else if (validityDays >= 30) validityScore = 30;
    else if (validityDays >= 15) validityScore = 20;
    else if (validityDays >= 7) validityScore = 10;
    else validityScore = 5;
  }

  // ── Délai de livraison (0-35 points) ──
  let deliveryScore = 0;
  if (deliveryDelayDays !== null) {
    if (deliveryDelayDays <= 7) deliveryScore = 35;
    else if (deliveryDelayDays <= 14) deliveryScore = 25;
    else if (deliveryDelayDays <= 30) deliveryScore = 15;
    else if (deliveryDelayDays <= 60) deliveryScore = 10;
  ...
  if (paymentTerms) {
    ...
    if (terms.includes("30 jours") || terms.includes("30 days"))
      ...
      paymentScore = 15;
    else paymentScore = 12; // Conditions non standard
  } else {
 ...
 */
export function areConditionsAcceptable(
  params: ConditionsScoreInput,
  minScore: number = 50,
