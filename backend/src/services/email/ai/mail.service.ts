...
const attachmentsDir =
  process.env.SUPPLIER_ATTACHMENTS_DIR ?? "uploads/supplier-responses";
...
async function handleParsedEmail(params: {
  ...
  const offerSupplier = await db.query.offerSuppliers.findFirst({
    where: and(
      eq(offerSuppliers.offerId, offer.id),
      eq(offerSuppliers.supplierId, supplier.id),
    ),
  });

  const negative = isNegativeSupplierResponse(
    `${params.subject}\n${params.text}`,
  );
...
async function saveAttachments(
  ...
  for (const attachment of attachments) {
    const originalFileName =
      sanitizeFileName(attachment.filename) || "attachment.bin";
...
async function findSupplierFromEmail(from: string) {
...
async function findOfferFromEmail(params: { subject: string; text: string }) {
  const haystack = `${params.subject}\n${params.text}`.toLowerCase();

  const uuid = haystack.match(
...
function createMessageIdFallback(uid: number, date?: Date) {
  return `fallback-${uid}-${date?.getTime() ?? Date.now()}`;
}

function isNegativeSupplierResponse(value: string) {
