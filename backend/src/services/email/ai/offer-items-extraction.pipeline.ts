...
import {
  buildOfferItemsPrompt,
  dedupeOfferItems,
  safeParseOfferItems,
} from "./AiOfferItemsExtraction";
...
export async function processOfferItemsFromAttachments(
  offerId: string,
  options: ProcessOfferItemsOptions = {},
) {
  ...
  for (const attachment of attachments) {
    try {
      ...
      console.error(
        `Failed extracting text from ${attachment.fileName}`,
        error,
      );
    }
  }
  ...
  const validItems = dedupeOfferItems(
    extraction.items.filter((item: any) => item?.name?.trim()),
  );
  ...
  await db.transaction(async (tx) => {
    ...
    for (let index = 0; index < validItems.length; index++) {
      ...
      await tx.insert(offerItems).values({
        ...
        minConformityPercentage:
          typeof item.minConformityPercentage === "number"
...
async function extractOfferItemsWithAi(text: string) {
  ...
  const completion = await openai.responses.create({
    ...
    instructions:
      "You extract structured biomedical tender or offer requested items.",
...
export function preCleanOcrText(text: string): string {
  return (
    text
