...
import {
  buildAttachmentClassificationPrompt,
  safeParseAttachmentClassification,
} from "./AiAttachmentClassifier";
...
export const aiRoute: Router = Router();
...
/* ════════════════════════════════════════════════════════════════════════
   JOB QUEUE — Gestion asynchrone des analyses PDF volumineux
   ════════════════════════════════════════════════════════════════════════ */
...
const jobs = new Map<string, TenderJob>();
...
async function processTenderExtraction(
  jobId: string,
  buffer: Buffer,
  filename: string,
): Promise<void> {
  ...
  try {
    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      console.log(
        `[Job ${jobId}] Processing chunk ${i + 1}/${chunks.length}: pages ${chunkIndices
          .map((p) => p + 1)
          .join(", ")}`,
      );
    ...
    if (mergedResult.tender?.lots?.length > 0) {
      console.log(
        `[Job ${jobId}] Post-processing: ${mergedResult.tender.lots.length} lots`,
      );

      // Nettoyer les objets de lots (supprimer texte système/technique)
      const cleanedLots = mergedResult.tender.lots.map((lot: any) => ({
        ...lot,
      ...
      console.log(
        `[Job ${jobId}] Generating Excel files: ${cleanedLots.length} lots`,
      );

      const lotsWithExcel = await Promise.all(
        cleanedLots.map(async (lot: any) => {
          ...
          return {
            ...lot,
            excelBase64,
            excelFileName: `lot_${sanitizeFileName(
              lot.number || "unknown",
            )}_dqe.xlsx`,
          };
        }),
...
*/
function cleanLotObject(object: string | null | undefined): string {
  if (!object) return "";
...
function mergeChunkResults(acc: any, chunk: any): any {
  ...
  if (t.lots?.length > 0) {
    const existingMap = new Map(
      merged.tender.lots.map((l: any) => [l.number, l]),
    );
    ...
    for (const lot of t.lots) {
      ...
      if (!existing) {
        ...
        if (lot.detailQuantitatifPages?.length > 0) {
          ...
          existing.detailQuantitatifPages = [...new Set(allPages)].sort(
        ...
        if (lot.object && lot.object.trim()) {
          ...
          // on prend le nouveau (probablement la vraie description)
            ...
            newObj.length < existingObj.length &&
...
function chooseBetterDescription(a: string, b: string): string {
...
function mergeDqeItems(existingItems: any[], newItems: any[]): any[] {
...
function buildDqeItemKey(item: any): string {
...
function chooseLonger(a: unknown, b: unknown): string | null {
...
async function generateLotExcelBase64({
  lot,
  tender,
}: {
  lot: any;
...
aiRoute.post("/equipment/search-queries", async (req, res) => {
  try {
    ...
    const result = await generateOptimizedQueries(
      equipmentName,
      categoryHint,
      count || 4,
    );
