...
import { detectProductFamily, normalizeText } from "./AiConformityResult";
...
export function buildOfferItemsPrompt(text: string) {
  return `
...
export function safeParseOfferItems(
  content: string,
): AiOfferItemsExtraction | null {
  try {
    ...
    return JSON.parse(cleaned) as AiOfferItemsExtraction;
  } catch (error) {
    console.error("[OFFER ITEMS JSON PARSE ERROR]", error);
    return null;
  }
}
function normalizeItemIdentity(value: string) {
  return value
    .toLowerCase()
    .normalize("NFD")
...
function areDuplicateOfferItems(a: any, b: any) {
  if (
    a.itemNumber &&
    b.itemNumber &&
...
export function dedupeOfferItems(items: any[]) {
  const result: any[] = [];

  for (const item of items) {
    const existingIndex = result.findIndex((existing) =>
      areDuplicateOfferItems(existing, item),
    );
