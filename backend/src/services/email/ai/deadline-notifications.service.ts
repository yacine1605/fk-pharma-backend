...
export async function checkUpcomingDeadlines() {
  ...
  const offersWithDeadlines = await db
    ...
    .where(
      or(
        and(
          gte(offers.technicalDepartmentDepositDate, today),
          lte(offers.technicalDepartmentDepositDate, in3Days),
        ),
        and(
          gte(offers.hospitalDepositDate, today),
          lte(offers.hospitalDepositDate, in7Days),
        ),
      ),
    );
  ...
  for (const { offer, entity } of offersWithDeadlines) {
    ...
    if (offer.technicalDepartmentDepositDate) {
      const deadline = startOfDay(
        new Date(offer.technicalDepartmentDepositDate),
      );
      ...
      if (daysUntil >= 0 && daysUntil <= 3) {
        const existing = await db
          ...
          .where(
            and(
              eq(notifications.offerId, offer.id),
              eq(notifications.type, "deadline_reminder"),
              eq(notifications.title, "Dépôt technique imminent"),
            ),
          );
        ...
        const existing = await db
          ...
          .where(
            and(
              eq(notifications.offerId, offer.id),
              eq(notifications.type, "deadline_reminder"),
              eq(notifications.title, "Dépôt hôpital imminent"),
            ),
          );
  ...
  const { supplierProformas, supplierResponses, suppliers } =
  ...
  const proformasNearExpiry = await db
    ...
    .where(
      and(
        gte(supplierProformas.validityDays, 1),
        lte(supplierProformas.validityDays, 7),
      ),
    );
  ...
  for (const { proforma, response, suppliers } of proformasNearExpiry) {
    const existing = await db
      ...
      .where(
        and(
          eq(notifications.supplierResponseId, response.id),
          eq(notifications.type, "deadline_reminder"),
          eq(notifications.title, "Proforma bientôt périmée"),
        ),
      );
  ...
  const customDeadlines = await db
    ...
    .where(
      and(
        lte(offerDeadlines.date, in3Days),
        gte(offerDeadlines.date, today),
        isNull(offerDeadlines.notifiedAt),
      ),
    );
...
export async function checkExpiredDeadlines() {
  ...
  for (const { offer, entity } of expiredTechnical) {
    const existing = await db
      ...
      .where(
        and(
          eq(notifications.offerId, offer.id),
          eq(notifications.type, "deadline_expired"),
          eq(notifications.title, "Date dépôt technique dépassée"),
        ),
      );
    ...
    const existing = await db
      ...
      .where(
        and(
          eq(notifications.offerId, offer.id),
          eq(notifications.type, "deadline_expired"),
          eq(notifications.title, "Date dépôt hôpital dépassée"),
        ),
      );
  ...
  const expiredCustom = await db
    ...
    .innerJoin(offers, eq(offerDeadlines.offerId, offers.id))
    .where(
      and(lt(offerDeadlines.date, today), isNull(offerDeadlines.notifiedAt)),
    );
