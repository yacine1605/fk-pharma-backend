...
import {
  enqueuePendingSupplierResponsesPipeline,
  processOfferAnalysisPipeline,
  processSupplierResponsePipeline,
  recomputeOfferSupplierRanking,
  runPendingSupplierResponsesPipeline,
} from "./analysis.pipeline";

export const analysisPipelineRouter: Router = Router();
...
analysisPipelineRouter.post(
  "/responses/:supplierResponseId/run",
  async (req, res) => {
    try {
...
analysisPipelineRouter.post(
  "/offers/:offerId/export-comparison",
  async (req, res) => {
    try {
      const { offerId } = req.params;

      const { filePath, rowCount } =
        await buildAndExportComparisonExcel(offerId);
