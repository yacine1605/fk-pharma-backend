...
import {
  ...
  generateBestOfferExcel,
  generateBestOfferExport,
  generateMissingItemsSheet,
  type BestOfferResult,
} from "./best-offer.service";

export const bestOfferRouter: Router = Router();
    ...
    return res.json({
      ...
      data: {
        ...
        lines: result.lines.map((line) => ({
          ...
          tvaAmount:
            line.unitPrice * line.marginRate * line.quantity * line.tvaRate,
          totalTTC:
            line.unitPrice *
            line.marginRate *
            line.quantity *
 ...
 */
bestOfferRouter.post(
  "/offers/:offerId/regenerate-missing",
  async (req, res) => {
    try {
      const { offerId } = req.params;
