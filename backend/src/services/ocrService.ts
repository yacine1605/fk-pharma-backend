...
async function extractWithMistralOCR(pdfBuffer: Buffer): Promise<string> {
  try {
    ...
    return ocrResponse.pages
      .map((p: { markdown: any; text: any }) => p.markdown || p.text)
      .join("\n\n");
  } catch (err: any) {
    if (err.code === "MODULE_NOT_FOUND") {
      throw new Error(
        "Module @mistralai/mistralai non installé. Lancez: npm install @mistralai/mistralai",
      );
    }
    throw err;
  }
...
async function structureWithAI(rawText: string, supplierName: string): Promise<any[]> {
...
Extrais tous les produits en JSON.`;
  ...
  const products: any[] = Array.isArray(parsed.products)
    ? parsed.products
...
async function extractProductsFromPDF(pdfBuffer: Buffer, supplierName: string): Promise<any[]> {
  ...
  if (rawText.trim().length < 100) {
    console.warn(
      `[OCR] ${supplierName}: PDF probablement scanné ou image (texte=${rawText.trim().length} chars).`,
    );
    ...
    if (process.env.MISTRAL_API_KEY) {
      ...
      console.warn(
        "[OCR] MISTRAL_API_KEY non configuré. Pour les scans, installez et configurez Mistral OCR.",
      );
    }
  }
  ...
  if (!rawText.trim()) {
    throw new Error(
      `Aucun texte extractible dans le PDF de ${supplierName}. Le fichier est peut-être un scan sans couche OCR.`,
    );
  }
