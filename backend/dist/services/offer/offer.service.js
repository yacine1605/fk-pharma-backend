...
import { medicalEntities, offerRecipients, offers, suppliers, } from "../../db/schema";
...
export async function createAndSendOffer(dto) {
    ...
    const selectedRecipients = dto.recipientIds.length > 0
        ? await db
            .select()
            .from(suppliers)
    ...
    await db.insert(offerRecipients).values(selectedRecipients.map((r) => ({
        offerId: offer.id,
        recipientId: r.id,
        emailStatus: "pending",
    })));
    ...
    for (const recipient of selectedRecipients) {
        try {
            ...
            await db
                ...
                .where(eq(offerRecipients.offerId, offer.id) &&
