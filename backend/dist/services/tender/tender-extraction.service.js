...
import { tenderDocuments, offerItems, offers, } from "../../db/schema";
import { extractTextSmart, extractTextFromDocx, extractTextFromExcel, } from "../email/ai/exctraction";
import { ocrImage } from "../email/ai/image-ocr";
import { documentVerificationQueue } from "../email/ai/queues";
const openai = new OpenAI({ apiKey: process.env.OPENAI_API_KEY });
const MODEL = "gpt-4o";
// ─── Main Pipeline ────────────────────────────────────────────────────────────
export async function processTenderDocument(documentId) {
    ...
    try {
        ...
        const itemsCreated = await populateOfferItems(doc.offerId, extracted.products, doc.uploadedBy ?? undefined);
        ...
        try {
            ...
            console.log(`[TENDER-EXTRACTION] Stamp verification enqueued for doc ${documentId}`);
        }
        catch (enqueueErr) {
            console.warn("[TENDER-EXTRACTION] Failed to enqueue stamp verification:", enqueueErr);
        }
        return {
            documentId,
            offerId: doc.offerId,
...
// ─── Text extraction ──────────────────────────────────────────────────────────
async function extractRawText(doc) {
...
function buildExtractionPrompt(text, fileName) {
    ...
    try {
        ...
        return {
            ...
            administrativeDocuments: Array.isArray(parsed.administrativeDocuments)
                ? parsed.administrativeDocuments.filter((d) => typeof d === "string")
                : [],
...
function normalizeProducts(rawProducts) {
    ...
    return rawProducts
        ...
        specifications: Array.isArray(p.specifications)
            ? p.specifications.filter((s) => typeof s === "string")
            : [],
    }));
}
// ─── Populate offerItems ──────────────────────────────────────────────────────
async function populateOfferItems(offerId, products, userId) {
    ...
    if (existingItems.length > 0) {
        console.log(`[TENDER-EXTRACTION] ${existingItems.length} articles déjà présents — extraction stockée sans écrasement.`);
        return 0;
    }
    // Insert new items
    ...
    console.log(`[TENDER-EXTRACTION] ${newItems.length} articles créés pour l'offre ${offerId}`);
    return newItems.length;
}
// ─── Update Offer Metadata ────────────────────────────────────────────────────
async function updateOfferMetadata(offerId, extracted) {
...
//# sourceMappingURL=tender-extraction.service.js.map