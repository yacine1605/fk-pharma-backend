...
export function startTenderExtractionWorker() {
    new Worker(tenderExtractionQueue.name, async (job) => {
        const { documentId } = job.data;
        console.log(`[TENDER-WORKER] Processing document: ${documentId}`);
        const result = await processTenderDocument(documentId);
        console.log(`[TENDER-WORKER] Done: ${result.itemsCreated} articles créés, confiance: ${(result.confidence * 100).toFixed(0)}%`);
        return result;
    }, {
...
//# sourceMappingURL=tender-extraction.worker.js.map