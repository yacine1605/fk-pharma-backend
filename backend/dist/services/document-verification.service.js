...
async function analyzeImageForStamp(base64Image, mimeType) {
...
async function analyzeMultiplePages(pdfBuffer, options = {}) {
...
// ── Main entry point ─────────────────────────────────────────────────────────
export async function analyzeDocumentForStamp(filePath, mimeType, options = {}) {
    ...
    if (ext === ".pdf" || mimeType === "application/pdf") {
        ...
        throw new Error(`Unsupported file format for stamp verification: ${ext}. Supported: PDF, PNG, JPG, WebP, BMP, TIFF.`);
    }
...
function validateStampType(value) {
    const valid = [
        "official_round",
        "rectangular",
        "ink_stamp",
...
function validateSignatureType(value) {
    const valid = ["handwritten", "digital_typed", "none"];
    return valid.includes(value)
        ? value
        : "none";
}
function validateQuality(value) {
    const valid = ["clear", "blurry", "unreadable"];
    return valid.includes(value)
        ? value
        : "unreadable";
}
