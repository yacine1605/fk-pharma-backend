...
import { notifications, offerItems, supplierGlobalAnalyses, supplierItemAnalyses, supplierProformaLines, supplierProformas, supplierResponseAttachments, supplierResponses, suppliers, } from "../../../db/schema";
...
function normalizeConformityResult(result, requestedQuantity) {
    ...
    const percentage = totalWeight > 0 ? round2((totalScore / totalWeight) * 100) : 0;
    const mandatoryMissingCount = result.details.filter((detail) => {
        return detail.required && Number(detail.score ?? 0) <= 0;
    }).length;
    const quantityRequested = Number(requestedQuantity ?? result.quantityRequested ?? 0);
    const quantityOffered = Number(result.quantityOffered ?? 0);
    const quantityOk = quantityRequested <= 0 ||
...
export async function processSupplierResponsePipeline(supplierResponseId, options) {
    ...
    const [claimed] = await db
        ...
        .where(and(eq(supplierResponses.id, supplierResponseId), inArray(supplierResponses.status, allowedStatuses), eq(supplierResponses.isNegativeResponse, false)))
        .returning();
    if (!claimed) {
    ...
    try {
        ...
        const technicalText = extractedAttachments
            .filter((item) => ["technical_sheet", "catalog", "document", "other", "image"].includes(item.attachmentType))
            .map((item) => {
            return [
        ...
        if (!proformaExtraction) {
            await markResponseAsNeedsReview(supplierResponseId, "AI extraction failed");
            return {
                supplierResponseId,
                status: "needs_review",
        ...
        const [createdProforma] = await db
            ...
            extractedJson: proformaExtraction,
            confidence: proformaExtraction.confidence ?? 0,
        })
            .returning();
        const requestedItems = response.offer.offerItems;
        if (requestedItems.length === 0) {
            await markResponseAsNeedsReview(supplierResponseId, "No offer items found. Extract offer items before supplier analysis.");
            await createNotification({
                ...
                message: "Aucun article n'a été extrait depuis le cahier des charges. Lancez l'extraction des articles avant l'analyse fournisseur.",
            });
            return {
                supplierResponseId,
                status: "needs_review",
        ...
        for (const line of proformaExtraction.lines ?? []) {
            ...
            const normalizedConformityResult = normalizeConformityResult(conformityResult, matchedOfferItem.requestedQuantity);
            if (normalizedConformityResult.quantityOffered !== null &&
            ...
            const conformityPercentage = normalizedConformityResult.conformityPercentage;
            await db.insert(supplierItemAnalyses).values({
                ...
                proposedProductName: conformityResult.proposedProductName ?? line.designation,
                ...
                mandatoryMissingCount: normalizedConformityResult.mandatoryMissingCount ?? 0,
                isTechnicallyCompliant: normalizedConformityResult.isTechnicallyCompliant ?? false,
...
export async function enqueuePendingSupplierResponsesPipeline() {
    const pendingResponses = await db
        ...
        .where(and(inArray(supplierResponses.status, ["received"]), eq(supplierResponses.isNegativeResponse, false)));
...
export async function processOfferAnalysisPipeline(offerId) {
    const responses = await db
        .select()
        .from(supplierResponses)
        .where(and(eq(supplierResponses.offerId, offerId), inArray(supplierResponses.status, ["received", "needs_review"]), eq(supplierResponses.isNegativeResponse, false)));
    const results = [];
...
async function extractProformaWithAi(text) {
...
function similarityScoreBidirectional(a, b) {
    return Math.max(similarityScore(a, b), similarityScore(b, a));
}
function stringifyTechnicalRequirements(value) {
...
function matchProformaLineToOfferItem(line, items) {
    const lineSearchText = [line.designation, line.code ?? ""].join(" ");
    const lineFamily = detectProductFamily(lineSearchText);
    if (line.code) {
        const byCode = items.find((item) => item.code?.toLowerCase() === line.code?.toLowerCase());
        if (byCode) {
            return byCode;
        }
    }
    ...
    for (const item of items) {
        ...
        const genericScore = genericEquivalenceScore(lineSearchText, itemSearchText);
        score = Math.max(score, genericScore);
        const genericEquivalent = areGenericEquivalent(lineSearchText, itemSearchText);
        if (genericEquivalent) {
            score += 0.25;
        }
        // Strong family bonus
        if (lineFamily !== "unknown" &&
            itemFamily !== "unknown" &&
...
function getEffectiveConformity(analysis) {
...
function isQuantitySufficient(analysis) {
...
function selectBestAnalysisPerOfferItem(rows) {
    const bestByOfferItem = new Map();
...
export async function recomputeOfferSupplierRanking(offerId) {
    await db
    ...
    const responses = await db
        ...
        .leftJoin(supplierProformas, eq(supplierProformas.supplierResponseId, supplierResponses.id))
        .where(and(eq(supplierResponses.offerId, offerId), inArray(supplierResponses.status, ["analyzed", "needs_review"]), eq(supplierResponses.isNegativeResponse, false)));
    const candidates = [];
    for (const row of responses) {
        ...
        const technicalScore = selectedRows.reduce((sum, item) => {
            if (!item.analysis) {
                return sum;
            }
        ...
        const totalHT = totalHTFromSelectedAnalyses || Number(row.proforma?.totalHT ?? 0);
        ...
        const blockingItems = selectedRows.filter((item) => {
            ...
            return (conformity < minConformity ||
    ...
    for (const candidate of candidates) {
        candidate.priceScore =
            minPrice > 0 && candidate.totalHT > 0
                ...
                : 0;
        candidate.globalScore = round2(candidate.technicalScore * 0.5 +
    ...
    for (let index = 0; index < globalRanking.length; index++) {
        ...
        const moinsDisantRank = moinsDisantRanking.findIndex((x) => x.supplierResponseId === candidate.supplierResponseId) + 1;
        const mieuxDisantRank = mieuxDisantRanking.findIndex((x) => x.supplierResponseId === candidate.supplierResponseId) + 1;
        await db.insert(supplierGlobalAnalyses).values({
            ...
            rank: index + 1,
            isBestSupplier: globalRanking.length > 0 &&
...
async function markResponseAsNeedsReview(supplierResponseId, reason) {
    await db
        .update(supplierResponses)
        .set({
        status: "needs_review",
...
//# sourceMappingURL=analysis.pipeline.js.map