...
import { offers, offerItems, suppliers, supplierResponses, supplierItemAnalyses, supplierProformaLines, supplierProformas, } from "../../../db/schema";
import { searchOnlineSuppliersBatch, // ← AJOUTÉ: Import batch search
 } from "./web-search";
// ─────────────────────────────────────────────
// 1. SELECT BEST SUPPLIER PER PRODUCT (CORRIGÉ N+1)
// ─────────────────────────────────────────────
export async function selectBestOffersForCahier(offerId, options = {}) {
    ...
    for (let idx = 0; idx < requestedItems.length; idx++) {
        ...
        let selected = await selectBestSupplierForItem(item, itemAnalyses, itemRawLines);
        if (selected) {
            lines.push({
                rowNumber: idx + 1,
    ...
    if (itemsNeedingSearch.length > 0) {
        console.log(`[BEST OFFER] ${itemsNeedingSearch.length} items need web search, using batch...`);
        const searchResults = await searchOnlineSuppliersBatch(itemsNeedingSearch.map((i) => ({ id: i.id, name: i.name })), 8);
        for (const { id, name, index } of itemsNeedingSearch) {
            ...
            if (enriched) {
                const aiRecText = enriched.aiRecommendations
                    .map((r) => `• ${r.name} [${r.likelihood.toUpperCase()}]: ${r.reason}`)
                    .join("\n");
                missingItems.push({
    ...
    // ── 6. Calculate totals ──
    const totalHT = lines.reduce((sum, l) => sum + l.unitPrice * l.marginRate * l.quantity, 0);
...
async function selectBestSupplierForItem(item, itemAnalyses, itemRawLines) {
    let selected = null;
    ...
    const compliantAnalyses = itemAnalyses.filter((a) => {
        ...
        const quantityOk = offeredQty >= requestedQty || a.analysis.quantityOffered === null;
    ...
    if (compliantAnalyses.length > 0) {
        ...
        selected = {
            ...
            hasTechnicalSheet: await checkTechnicalSheet(best.analysis.supplierResponseId, item.id),
    ...
    // Priority 2: Any AI analysis
        ...
        const best = itemAnalyses.sort((a, b) => Number(b.analysis.conformityPercentage ?? 0) -
            Number(a.analysis.conformityPercentage ?? 0))[0];
        selected = {
            supplierName: best.supplier.name,
            unitPrice: Number(best.analysis.unitPriceHT ?? 0),
            quantity: best.analysis.quantityOffered ?? item.requestedQuantity ?? 1,
    ...
    // Priority 3: Raw proforma lines
        ...
        const best = itemRawLines.sort((a, b) => Number(a.line.unitPriceHT ?? Infinity) -
            Number(b.line.unitPriceHT ?? Infinity))[0];
        selected = {
            supplierName: best.supplier.name,
...
async function checkTechnicalSheet(supplierResponseId, offerItemId) {
    const { supplierResponseAttachments } = await import("../../../db/schema");
    const attachments = await db
        .select()
        .from(supplierResponseAttachments)
        .where(and(eq(supplierResponseAttachments.supplierResponseId, supplierResponseId), eq(supplierResponseAttachments.attachmentType, "technical_sheet")));
    return attachments.length > 0;
}
// ─────────────────────────────────────────────
// 2. EXCEL GENERATION (Exact Reference Format)
// ─────────────────────────────────────────────
export async function generateBestOfferExcel(result) {
...
// ─────────────────────────────────────────────
export async function generateMissingItemsSheet(workbook, missingItems) {
...
// ─────────────────────────────────────────────
export async function generateBestOfferExport(offerId) {
...
//# sourceMappingURL=best-offer.service.js.map