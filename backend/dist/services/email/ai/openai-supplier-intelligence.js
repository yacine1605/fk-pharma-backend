...
function safeLogError(label, err) {
    try {
        ...
        if (err instanceof Error) {
            ...
            if ("response" in err) {
                try {
                    safe.response = JSON.stringify(err.response, null, 2);
                }
                catch {
...
// ─────────────────────────────────────────────
async function aiParse(prompt, schema, systemContent) {
...
// ─────────────────────────────────────────────
export async function extractSuppliersWithAI(searchResults, equipmentQuery) {
    if (!process.env.OPENAI_API_KEY) {
...
`;
    const parsed = await aiParse(prompt, SupplierExtractionSchema, "Tu es un assistant d'extraction de données spécialisé dans les fournisseurs de matériel médical algériens.");
    if (!parsed || !parsed.suppliers) {
        return searchResults.flatMap((r) => legacyExtract(r.title, r.snippet));
    }
    return parsed.suppliers
        .filter((s) => s.isAlgerian && s.confidence >= 0.3)
        .map((s) => ({
...
// ─────────────────────────────────────────────
export async function generateOptimizedQueries(equipmentName, categoryHint, count = 4) {
    if (!process.env.OPENAI_API_KEY) {
        return [
...
// ─────────────────────────────────────────────
export async function classifyEquipment(description) {
    if (!process.env.OPENAI_API_KEY) {
        return {
            category: "Diagnostic",
            subcategory: "Général",
...
`;
    const parsed = await aiParse(prompt, EquipmentClassificationSchema, "Tu es un expert en marchés publics algériens de matériel médical.");
    return (parsed || {
        category: "Diagnostic",
        subcategory: "Général",
        frenchSearchTerms: [description],
...
// ─────────────────────────────────────────────
export async function recommendSuppliersForMissingItem(itemName, classification) {
    if (!process.env.OPENAI_API_KEY)
        return [];
    const prompt = `
...
// ─────────────────────────────────────────────
function legacyExtract(title, snippet) {
    const text = `${title} ${snippet}`;
    const results = [];
    const legalMatch = text.match(/(SARL|EURL|SPA|ETS|GIE|SNC|SCS)\s+([A-Za-z0-9\s&\-\.]+)/i);
    const cityMatch = text.match(/\b(Alger|Algiers|Constantine|Oran|Tlemcen|Annaba|Blida|Setif|Sétif|Béjaïa|Batna|Biskra|Boumerdès)\b/i);
...
//# sourceMappingURL=openai-supplier-intelligence.js.map