...
import { extractSuppliersWithAI, generateOptimizedQueries, classifyEquipment, recommendSuppliersForMissingItem, } from "./openai-supplier-intelligence";
...
// ─────────────────────────────────────────────
export async function searchOnlineSuppliersBatch(items, maxResultsPerItem = 5) {
    const results = new Map();
    for (let i = 0; i < items.length; i += MAX_CONCURRENT_SEARCHES) {
        const chunk = items.slice(i, i + MAX_CONCURRENT_SEARCHES);
        const chunkResults = await Promise.all(chunk.map(async (item) => {
            const result = await searchOnlineSuppliersAI(item.name, maxResultsPerItem);
...
// ─────────────────────────────────────────────
export async function performBingSearch(query, maxResults = 5) {
    ...
    try {
        const controller = new AbortController();
        const timeout = setTimeout(() => controller.abort(), 15000);
        const response = await fetch(`https://api.bing.microsoft.com/v7.0/search?q=${encodeURIComponent(query)}&count=${maxResults}&mkt=fr-FR`, {
...
// ─────────────────────────────────────────────
export async function searchOnlineSuppliersAI(itemName, maxResults = 8) {
    const classification = await classifyEquipment(itemName);
    const queries = await generateOptimizedQueries(itemName, classification.subcategory, 3);
    const searchPromises = queries.map(async (q) => {
        const serpResults = await performWebSearch(q, Math.ceil(maxResults / queries.length));
    ...
    const structuredSuppliers = await extractSuppliersWithAI(uniqueResults, itemName);
    const aiRecommendations = await recommendSuppliersForMissingItem(itemName, classification);
    const structuredNames = structuredSuppliers.map((s) => s.legalForm && !s.companyName.toUpperCase().startsWith(s.legalForm)
...
// ─────────────────────────────────────────────
export function extractCompanyName(title, snippet) {
