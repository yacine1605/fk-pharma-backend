...
function headerStyle(ws, cell, bg = COLORS.headerBg, fg = COLORS.headerFg) {
...
// MAIN GENERATOR
// ─────────────────────────────────────────────
export async function generateComparisonExcel(options) {
...
// ─────────────────────────────────────────────
async function buildComparisonSheet(workbook, title, rows, offerMeta) {
    ...
    for (const [section, span] of Object.entries(sectionSpans)) {
        ...
        cell.fill = {
            type: "pattern",
            pattern: "solid",
            fgColor: { argb: SECTION_COLORS[section] },
        };
    ...
    rows.forEach((row, idx) => {
        ...
        if (idx % 2 === 1) {
            for (let c = 1; c <= COLUMNS.length; c++) {
                ...
                if (!cell.fill ||
                    cell.fill.fgColor?.argb === undefined) {
                    cell.fill = {
                        type: "pattern",
                        pattern: "solid",
...
// FEUILLE 2 : SYNTHÈSE
// ─────────────────────────────────────────────
async function buildSummarySheet(workbook, rows) {
    ...
    for (const row of rows) {
        ...
        if (row.technicalSheetStatus === "existe")
            existing.fichesOk++;
        if (row.isLeastExpensive)
            existing.isLeast = true;
        if (row.isBestValue)
    ...
    for (const [name, agg] of bySupplier) {
        const avgConformity = agg.conformityCount > 0
...
function buildRowValues(row) {
    return {
        ...
        deliveryDelayText: (row.deliveryDelayText ?? row.deliveryDelayDays)
...
//# sourceMappingURL=comparisonExcel.js.map