...
function getGenericCanonicalHits(value) {
...
export function canonicalizeGenericText(value) {
...
export function genericEquivalenceScore(a, b) {
    ...
    const lexicalScore = Math.max(similarityScore(canonicalA, canonicalB), similarityScore(canonicalB, canonicalA));
    if (hasCommonGeneric) {
        return Math.max(0.75, lexicalScore);
    }
    return lexicalScore;
}
export function areGenericEquivalent(a, b) {
...
export function safeParseConformity(content) {
    try {
...
export function detectProductFamily(value) {
    const text = normalizeText(value);
...
//# sourceMappingURL=AiConformityResult.js.map