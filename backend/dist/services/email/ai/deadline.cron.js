...
import { checkUpcomingDeadlines, checkExpiredDeadlines, } from "./deadline-notifications.service";
...
export function startDeadlineCron() {
    ...
    cron.schedule(DEADLINE_CHECK_SCHEDULE, async () => {
        ...
        try {
            ...
            console.log(`[CRON] Deadline check complete: ${result.notificationsCreated} notifications created`);
        }
        catch (error) {
    ...
    cron.schedule(EXPIRED_CHECK_SCHEDULE, async () => {
        ...
        try {
            ...
            console.log(`[CRON] Expired check complete: ${result.expiredCount} expired deadlines found`);
        }
        catch (error) {
...
//# sourceMappingURL=deadline.cron.js.map