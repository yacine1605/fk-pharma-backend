...
import { offerItems, offers, supplierGlobalAnalyses, supplierItemAnalyses, supplierProformas, supplierProformaLines, supplierResponseAttachments, supplierResponses, suppliers, } from "../../../db/schema";
import { generateComparisonExcel, } from "./comparisonExcel";
// ─────────────────────────────────────────────
 ...
 */
export async function buildAndExportComparisonExcel(offerId) {
    ...
    const allItemAnalyses = await db
        ...
        .where(inArray(supplierItemAnalyses.supplierResponseId, responses.map((r) => r.response.id)));
    ...
    const technicalAttachments = await db
        ...
        .where(and(inArray(supplierResponseAttachments.supplierResponseId, responses.map((r) => r.response.id)), inArray(supplierResponseAttachments.attachmentType, [
        "technical_sheet",
        "catalog",
    ])));
    const technicalSheetByResponse = new Set(technicalAttachments.map((a) => a.supplierResponseId));
    // Build rows
    const rows = [];
    for (const { response, supplier, proforma } of responses) {
        const responseAnalyses = allItemAnalyses.filter((a) => a.analysis.supplierResponseId === response.id);
        ...
        for (const { analysis, offerItem, proformaLine } of responseAnalyses) {
            const conformityPercentage = analysis.manualOverride && analysis.manualConformityPercentage !== null
                ? Number(analysis.manualConformityPercentage)
                : Number(analysis.conformityPercentage ?? 0);
            const unitPriceHT = Number(proformaLine?.unitPriceHT ?? analysis.unitPriceHT ?? 0);
            const totalHT = Number(proformaLine?.totalHT ?? analysis.totalHT ?? 0);
            const tvaRate = Number(proformaLine?.tvaPercentage ?? analysis.tvaPercentage ?? 0);
            ...
            const deliveryDelayDays = extractDaysFromText(response.deliveryDelay ?? offer.deliveryDelay);
            // Warranty — prefer response > offer
            const warrantyText = response.warrantyDuration ?? offer.warrantyDuration ?? null;
            const warrantyMonths = extractMonthsFromText(warrantyText);
            // SAV — prefer response > offer
            const afterSalesService = response.afterSalesService ??
...
// HELPERS
// ─────────────────────────────────────────────
function resolveMoinsDisant(globalAnalyses) {
    const eligible = globalAnalyses.filter((a) => Number(a.totalHT) > 0);
    if (eligible.length === 0)
        return null;
    const sorted = [...eligible].sort((a, b) => Number(a.totalHT) - Number(b.totalHT));
    return sorted[0].supplierResponseId;
}
function resolveMieuxDisant(globalAnalyses) {
    const eligible = globalAnalyses.filter((a) => a.isEligible);
    if (eligible.length === 0) {
        // Fallback: highest global score regardless of eligibility
        const sorted = [...globalAnalyses].sort((a, b) => Number(b.globalScore) - Number(a.globalScore));
        return sorted[0]?.supplierResponseId ?? null;
    }
    const sorted = [...eligible].sort((a, b) => Number(b.globalScore) - Number(a.globalScore));
    return sorted[0].supplierResponseId;
}
...
function buildSavText(duration, locations) {
    const parts = [
