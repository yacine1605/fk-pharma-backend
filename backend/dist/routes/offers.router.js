...
import { createAndSendOffer, saveOfferDraft, listOffers, getOfferById, } from "../services/offer/offerService";
...
import { medicalEntities, offerAttachments, offerDocumentFolderFiles, offerDocumentFolders, offerExcelExports, offerItems, offerRecipients, offerSuppliers, offers, supplierGlobalAnalyses, supplierItemAnalyses, supplierProformaLines, supplierProformas, supplierResponseAttachments, supplierResponses, suppliers, } from "../db/schema";
import { processOfferAnalysisPipeline, recomputeOfferSupplierRanking, } from "../services/email/ai/analysis.pipeline";
import { generateBestOfferExport, selectBestOffersForCahier, } from "../services/email/ai/best-offer.service";
...
async function resolveSourceAttachment(sourceOfferId) {
...
async function getOfferAttachments(offerId) {
    return db
        .select()
...
export async function getTechnicalSheetStatuses(responseIds) {
    ...
    const rows = await db
        ...
        .where(and(inArray(supplierResponseAttachments.supplierResponseId, responseIds), eq(supplierResponseAttachments.attachmentType, "technical_sheet")));
...
const requireOfferAccess = async (req, res, next) => {
    try {
        ...
        const [recipient] = await db
            ...
            .where(and(eq(offerRecipients.offerId, offerId), eq(offerRecipients.recipientId, userId)))
            .limit(1);
        if (recipient) {
            next();
            return;
        ...
        const [supplierLink] = await db
            ...
            .where(and(eq(offerSuppliers.offerId, offerId) /*eq(suppliers.userId, userId)*/))
            .limit(1);
        if (supplierLink) {
            next();
            return;
...
router.get("/", async (req, res, next) => {
    try {
        ...
        const myOffers = await db
            ...
            .innerJoin(medicalEntities, eq(offers.medicalEntityId, medicalEntities.id))
...
// ═══════════════════════════════════════════════════════════════════════════════
router.get("/all", requireRole(["admin", "technique"]), async (_req, res, next) => {
    try {
        const result = await listOffers();
        return res.json({ success: true, data: result });
    }
...
router.get("/stats", requireRole(["admin", "agent_commercial", "technique"]), async (_req, res) => {
...
router.get("/document-folders", requireRole(["admin", "agent_commercial"]), async (req, res) => {
    try {
        const { search, offerId } = req.query;
        const conditions = [];
        if (typeof search === "string" && search.trim().length > 0) {
            conditions.push(sql `${offerDocumentFolders.name} ILIKE ${"%" + search.trim() + "%"}`);
        }
        ...
        const countMap = new Map(fileCounts.map((c) => [c.folderId, Number(c.count)]));
...
router.post("/send", requireRole(["admin", "agent_commercial", "technique", "distributor"]), upload.array("attachments", 20), async (req, res, next) => {
    try {
        let rawData;
        try {
        ...
        const attachmentTypes = (req.body.attachmentTypes ?? []);
        if (files.length > 0) {
            ...
            const sourceAttachments = await getOfferAttachments(rawData.sourceOfferId);
            if (sourceAttachments && sourceAttachments.length > 0) {
                rawData.attachments = sourceAttachments.map((att) => ({
                ...
                const sourceAttachment = await resolveSourceAttachment(rawData.sourceOfferId);
                if (sourceAttachment) {
                    rawData.attachments = [
                        {
...
router.post("/draft", requireRole(["admin", "agent_commercial", "technique", "distributor"]), upload.array("attachments", 20), async (req, res, next) => {
    try {
        let rawData;
        try {
            rawData = JSON.parse(req.body.data ?? "{}");
...
router.post("/send-commercial", requireRole(["admin", "agent_commercial"]), upload.array("attachments", 10), async (req, res) => {
    try {
...
// ═══════════════════════════════════════════════════════════════════════════════
router.put("/:offerId/status", requireOfferAccess, requireRole(["admin", "agent_commercial"]), async (req, res) => {
    try {
    ...
    try {
        ...
        if (responseIds.length > 0) {
            attachments = await db
                ...
                .from(supplierResponseAttachments)
                .where(inArray(supplierResponseAttachments.supplierResponseId, responseIds))
                .orderBy(sql `${supplierResponseAttachments.createdAt} asc`);
        }
        const attachmentsByResponseId = new Map();
        for (const attachment of attachments) {
            const current = attachmentsByResponseId.get(attachment.supplierResponseId);
            if (current) {
        ...
        const responsesWithAttachments = responses.map((response) => {
            const responseAttachments = attachmentsByResponseId.get(response.id) ?? [];
            return {
                ...response,
        ...
        const proformaReceivedCount = responsesWithAttachments.filter((r) => r.attachments.some((a) => a.attachmentType === "proforma")).length;
        // Nombre de réponses négatives (non disponible)
        const negativeResponseCount = responses.filter((r) => r.isNegativeResponse === true).length;
...
router.get("/:id/attachment", requireOfferAccess, async (req, res, next) => {
    try {
...
router.get("/:offerId/responses/:responseId/attachments/:attachmentId/download", requireOfferAccess, async (req, res) => {
    try {
        ...
        const [attachment] = await db
            ...
            .from(supplierResponseAttachments)
            .innerJoin(supplierResponses, eq(supplierResponseAttachments.supplierResponseId, supplierResponses.id))
            .where(and(eq(supplierResponses.offerId, offerId), eq(supplierResponses.id, responseId), eq(supplierResponseAttachments.id, attachmentId)));
        if (!attachment) {
            return res.status(404).json({
                success: false,
...
router.get("/:offerId/documents", requireOfferAccess, async (req, res) => {
    try {
        const { offerId } = req.params;
        const offer = await db.query.offers.findFirst({
            where: eq(offers.id, offerId),
        ...
        for (const attachment of receivedAttachments) {
            ...
            if (!receivedBySupplier.has(folderId)) {
                receivedBySupplier.set(folderId, {
                    id: folderId,
                    name: `${attachment.supplierName} - ${new Date(attachment.receivedAt).toLocaleDateString("fr-DZ")}`,
                    kind: "supplier",
...
router.get("/:id", requireOfferAccess, async (req, res, next) => {
    try {
        const offer = await getOfferById(req.params.id);
        if (!offer) {
            return res
...
router.get("/:offerId/comparison", requireOfferAccess, async (req, res) => {
    try {
        ...
        if (!offer)
            return res.status(404).json(err("Offre non trouvée"));
        ...
        const itemAnalyses = await db
            ...
            .innerJoin(supplierResponses, eq(supplierItemAnalyses.supplierResponseId, supplierResponses.id))
            .innerJoin(suppliers, eq(supplierResponses.supplierId, suppliers.id))
            .innerJoin(offerItems, eq(supplierItemAnalyses.offerItemId, offerItems.id))
            .leftJoin(supplierProformaLines, eq(supplierItemAnalyses.proformaLineId, supplierProformaLines.id))
            .where(and(eq(supplierResponses.offerId, offerId), inArray(supplierItemAnalyses.status, ["completed", "needs_review"])));
        const rawLines = await db
            ...
            .innerJoin(supplierProformas, eq(supplierProformaLines.proformaId, supplierProformas.id))
        ...
        for (const row of rawLines) {
            ...
            const entry = supplierMap.get(sid);
            const exists = entry.items.find((i) => i.offerItemId === row.offerItem?.id);
            if (!exists) {
                entry.items.push({
        ...
        const suppliersList = Array.from(supplierMap.values()).sort((a, b) => (a.rank || 999) - (b.rank || 999));
        const bestSupplier = suppliersList.find((s) => s.isBestSupplier) ||
            suppliersList[0] ||
            null;
        return res.json(ok({
            offerId,
...
//   }
// });
router.post("/:offerId/analyze", requireOfferAccess, requireRole(["admin", "agent_commercial"]), async (req, res) => {
    try {
        const { offerId } = req.params;
...
router.get("/:offerId/analyze/:jobId/status", requireOfferAccess, async (req, res) => {
    const { jobId } = req.params;
    const job = jobStore.get(jobId);
    if (!job)
        return res.status(404).json(err("Job introuvable"));
...
router.post("/:offerId/export", requireOfferAccess, async (req, res) => {
    try {
        ...
        await db
            ...
            .where(and(eq(offerExcelExports.offerId, offerId), eq(offerExcelExports.fileName, result.fileName)));
...
router.post("/:offerId/export/preview", requireOfferAccess, async (req, res) => {
    try {
        const { offerId } = req.params;
...
router.post("/:offerId/regenerate-missing", requireOfferAccess, async (req, res) => {
    try {
        const { offerId } = req.params;
        const result = await selectBestOffersForCahier(offerId);
        ...
        const enriched = await Promise.all(result.missingItems.map(async (item) => ({
            ...item,
...
router.post("/:offerId/document-folders", requireOfferAccess, async (req, res) => {
    try {
        const { offerId } = req.params;
            ...
            const rows = files.map((file) => {
                if (file.source === "received") {
                    return {
                        ...
                        offerId,
                        source: "received",
                        supplierResponseAttachmentId: file.attachmentId || file.id || null,
...
router.get("/:offerId/document-folders", requireOfferAccess, async (req, res) => {
    try {
        const { offerId } = req.params;
        const offer = await db.query.offers.findFirst({
            where: eq(offers.id, offerId),
        ...
        if (folderIds.length > 0) {
            files = await db
                ...
                supplierResponseAttachmentId: offerDocumentFolderFiles.supplierResponseAttachmentId,
                ...
                .from(offerDocumentFolderFiles)
                .leftJoin(supplierResponseAttachments, eq(offerDocumentFolderFiles.supplierResponseAttachmentId, supplierResponseAttachments.id))
                .leftJoin(supplierResponses, eq(supplierResponseAttachments.supplierResponseId, supplierResponses.id))
...
router.delete("/:offerId/document-folders/:folderId", requireOfferAccess, async (req, res) => {
    try {
        const { offerId, folderId } = req.params;
        const [folder] = await db
            .select()
            .from(offerDocumentFolders)
            .where(and(eq(offerDocumentFolders.id, folderId), eq(offerDocumentFolders.offerId, offerId)));
        if (!folder)
            return res.status(404).json(err("Dossier non trouvé"));
        await db
...
router.put("/:offerId/document-folders/:folderId", requireOfferAccess, async (req, res) => {
    try {
        const { offerId, folderId } = req.params;
        const { name } = req.body;
        if (!name || typeof name !== "string" || name.trim().length === 0) {
...
router.delete("/:offerId/document-folders/:folderId/files/:fileId", requireOfferAccess, async (req, res) => {
    try {
        const { offerId, folderId, fileId } = req.params;
        const [file] = await db
            .select()
            .from(offerDocumentFolderFiles)
            .where(and(eq(offerDocumentFolderFiles.id, fileId), eq(offerDocumentFolderFiles.folderId, folderId), eq(offerDocumentFolderFiles.offerId, offerId)));
        if (!file)
...
//# sourceMappingURL=offers.router.js.map