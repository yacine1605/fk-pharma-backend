...
function buildSignature(role, firstName, lastName) {
...
router.get("/", async (req, res) => {
    try {
        ...
        if (search) {
            query = query.where(or(ilike(users.firstName, `%${search}%`), ilike(users.lastName, `%${search}%`), ilike(users.email, `%${search}%`), ilike(users.company, `%${search}%`)));
        }
        if (role) {
...
router.put("/:id", async (req, res) => {
    try {
        const { id } = req.params;
        const { email, password, firstName, lastName, company, role, signature, phone, } = req.body;
