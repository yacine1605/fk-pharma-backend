...
import { tenderDocuments, offers, offerItems, } from "../db/schema";
 ...
 */
router.post("/:offerId/upload", requireRole(["admin", "agent_commercial", "technique"]), upload.single("file"), async (req, res) => {
    try {
        ...
        await tenderExtractionQueue.add("extract-tender", { documentId: doc.id }, {
            jobId: `tender-${doc.id}`,
            attempts: 3,
    ...
    try {
        ...
        return res.json(ok(docs.map((d) => ({
            ...
            uploadedBy: d.uploadedBy
                ? { id: d.uploadedBy.id, email: d.uploadedBy.email }
                : null,
 ...
 */
router.post("/document/:documentId/reprocess", requireRole(["admin", "agent_commercial", "technique"]), async (req, res) => {
    try {
        const { documentId } = req.params;
        const doc = await db.query.tenderDocuments.findFirst({
            where: eq(tenderDocuments.id, documentId),
        ...
        // Re-enqueue
        await tenderExtractionQueue.add("extract-tender", { documentId }, {
 ...
 */
router.post("/document/:documentId/apply", requireRole(["admin", "agent_commercial", "technique"]), async (req, res) => {
    try {
        const { documentId } = req.params;
        const doc = await db.query.tenderDocuments.findFirst({
            where: eq(tenderDocuments.id, documentId),
        ...
        const products = doc.extractedJson.products ?? [];
        ...
        const newItems = products.map((p) => ({
            ...
            technicalRequirements: (p.specifications ?? []).map((spec) => ({
                label: spec,
                required: true,
 ...
 */
router.delete("/document/:documentId", requireRole(["admin", "agent_commercial"]), async (req, res) => {
    try {
        const { documentId } = req.params;
        const doc = await db.query.tenderDocuments.findFirst({
            where: eq(tenderDocuments.id, documentId),
...
//# sourceMappingURL=tender-documents.js.map